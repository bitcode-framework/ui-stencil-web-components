...
// Run executes the go-json CLI with the given options.
// Call ApplyOptions before Run to register script runtimes.
func Run(opts ...CLIOption) {
	ApplyOptions(opts...)
	if len(os.Args) < 2 {
		printUsage()
		os.Exit(1)
...
func cmdRun(args []string) {
	...
	opts = append(opts, ExtraRuntimeOpts()...)
...
func cmdServe(args []string) {
	...
	rtOpts = append(rtOpts, ExtraRuntimeOpts()...)
