...
import {
  echarts, ECharts,
  parseChartData, isRadarData, getColorList,
...
@Component({ tag: 'bc-chart-radar', styleUrl: 'bc-chart-radar.css', shadow: false })
export class BcChartRadar {
  ...
  private renderChart() {
    ...
    if (!isRadarData(parsed)) return;
    const rd = parsed as RadarData;
    ...
    const option: Record<string, unknown> = {
      ...
      series: [{
        type: 'radar',
        data: rd.series.map(s => ({
          name: s.name,
          value: s.data,
          ...(this.filled ? { areaStyle: {} } : {}),
        })),
      }],
    };
