...
import {
  echarts, ECharts,
  parseChartData, isHeatmapArray, isHeatmapStructured, getColorList,
...
export class BcChartHeatmap {
  ...
  @Prop() locale: string = '';
  @Prop() visualMapMin: number = 0;
  @Prop() visualMapMax: number = 100;
  ...
  private renderChart() {
    ...
    const option: Record<string, unknown> = {
      ...
      color: colorList,
      visualMap: {
        min: this.visualMapMin, max: this.visualMapMax, calculable: true,
        orient: 'horizontal', left: 'center', bottom: '15%',
      },
    };

    if (isHeatmapStructured(parsed)) {
      const sd = parsed as HeatmapStructuredData;
      option.xAxis = { type: 'category', data: sd.xAxis, splitArea: { show: true } };
      option.yAxis = { type: 'category', data: sd.yAxis, splitArea: { show: true } };
      option.grid = buildGridOption(this.dataZoom, this.legend);
      const flatData: Array<[number, number, number]> = [];
      sd.data.forEach((row, yi) => {
        row.forEach((val, xi) => { flatData.push([xi, yi, val]); });
      });
      option.series = [{ type: 'heatmap', data: flatData, emphasis: { itemStyle: { shadowBlur: 10 } } }];
    } else if (isHeatmapArray(parsed)) {
      option.grid = { height: '50%', top: '10%' };
      option.series = [{ type: 'heatmap', data: parsed, emphasis: { itemStyle: { shadowBlur: 10 } } }];
    } else {
      option.grid = { height: '50%', top: '10%' };
      option.series = [{ type: 'heatmap', data: parsed, emphasis: { itemStyle: { shadowBlur: 10 } } }];
    }
