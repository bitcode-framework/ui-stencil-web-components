...
import {
  echarts, ECharts,
  parseChartData, isHierarchical, getColorList,
...
@Component({ tag: 'bc-chart-treemap', styleUrl: 'bc-chart-treemap.css', shadow: false })
export class BcChartTreemap {
  ...
  private renderChart() {
    ...
    if (!isHierarchical(parsed)) return;
    const tree = parsed as HierarchicalData;
    ...
    const option: Record<string, unknown> = {
      ...
      series: [{
        type: 'treemap',
        data: [tree],
        leafDepth: this.leafDepth || undefined,
        roam: this.roam,
      }],
    };
