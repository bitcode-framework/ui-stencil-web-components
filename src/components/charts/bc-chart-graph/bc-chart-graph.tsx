...
import {
  echarts, ECharts,
  parseChartData, isGraphData, getColorList,
...
@Component({ tag: 'bc-chart-graph', styleUrl: 'bc-chart-graph.css', shadow: false })
export class BcChartGraph {
  ...
  private renderChart() {
    ...
    if (!isGraphData(parsed)) return;
    const gd = parsed as GraphData;
    ...
    const option: Record<string, unknown> = {
      ...
      series: [{
        ...
        data: gd.nodes.map(n => ({
          ...n,
          symbolSize: n.value ? Math.max(10, Math.min(50, n.value)) : 20,
        })),
        links: gd.links,
