...
echarts.use([CandlestickChart, LineChart, MarkLineComponent, MarkPointComponent]);

function calculateMA(data: Array<[number, number, number, number]>, period: number): Array<number | '-'> {
...
@Component({ tag: 'bc-chart-candlestick', styleUrl: 'bc-chart-candlestick.css', shadow: false })
export class BcChartCandlestick {
  ...
  private renderChart() {
    ...
    const seriesList: Array<Record<string, unknown>> = [
      { type: 'candlestick', data: cd.data },
    ];

    if (this.showMA) {
      const maData = calculateMA(cd.data, this.maPeriod);
      seriesList.push({
        type: 'line',
        name: `MA${this.maPeriod}`,
        data: maData,
        smooth: true,
