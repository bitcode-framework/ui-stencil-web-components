...
export class BcChartPie {
  ...
  @Prop() locale: string = '';
  @Prop() donut: boolean = true;
  @Prop() roseType: string = '';
  ...
  private renderChart() {
    ...
    let pieData: Array<Record<string, unknown>> = [];

    if (isSingleSeries(parsed)) {
      pieData = (parsed as SingleSeriesItem[]).map(d => ({ name: d.name, value: d.value }));
    } else if (Array.isArray(parsed)) {
      pieData = parsed as Array<Record<string, unknown>>;
    }

    const radius: string[] = this.donut ? ['40%', '70%'] : ['0%', '70%'];
    const series: Record<string, unknown> = {
      type: 'pie', radius, data: pieData,
      emphasis: { itemStyle: { shadowBlur: 10 } },
    };
    if (this.roseType === 'radius' || this.roseType === 'area') {
