...
├── packages/
│   ├── components/      Stencil Web Components (@bitcode/components, 122 components, 11 groups)
...
- [x] Stencil Web Components — 122 components (fields, layout, views, charts, dialogs, widgets, media viewers/players, search, social, print)
- [x] Component registry — `registry.ts` with dependency graph, include/exclude selector, per-group exports
- [x] Theme system — `BcSetup.setTheme/getTheme/onThemeChange`, light.css + dark.css, CSS custom properties
- [x] RTL support — CSS logical properties, `BcSetup.setRtl/isRtl`, `--bc-direction` CSS var
- [x] Export architecture — Post-build `generate-exports.mjs` creates 151 package.json export paths (per-group JS, per-group CSS, per-component CSS, themes)
