...
8. **Web Components** — 122 Stencil.js components across 11 groups: 36 field types, layout, views, 26 charts (23 ECharts + 3 pure CSS), dialogs, widgets (incl. 8 media viewers/players), search, social, print. Component registry with dependency graph, include/exclude selector. Per-group/per-component CSS exports. Theme system (light/dark/system). RTL support.
...
| 3 | Model Relationships | ✅ | — | `many2one` (FK column), `one2many` (reverse FK), `many2many` (auto junction table), `morph_to` (polymorphic belongs-to), `morph_one` (polymorphic has-one), `morph_many` (polymorphic has-many), `morph_to_many` (polymorphic M2M parent), `morph_by_many` (polymorphic M2M inverse). Morph map for type aliasing. `MigrateModel()` handles DDL. | — |
...
| # | Feature | Status | Effort | What Exists | What's Missing |
|---|---------|--------|--------|-------------|----------------|
| 30 | Form Builder (Visual) | ⚠️ | L | JSON-based form layout (rows, fields, widths, tabs). SSR rendering. 122 Stencil.js components. | Visual drag-and-drop form designer in browser. |
...
| 72 | Enterprise Component Infrastructure | ✅ | — | All phases complete. BcSetup (global config + reactivity + theme + RTL runtime), 4-level data fetching, 3-level validation, theming (light/dark/system/custom via setTheme), RTL support (CSS logical properties + setRtl), 122 components across 11 groups, component registry with dependency graph and include/exclude, post-build export generation (151 paths), per-group/per-component CSS, 26 charts with enterprise features (modular ECharts, multi-series, shared chart-utils). All standalone — no BitCode dependency. | — |
...
The `@bitcode/components` Stencil library (122 components across 11 groups) provides rich UI widgets. Many are **built but not yet wired** to the engine's SSR rendering:

| Category | Components | Count | Engine Integration |
|----------|------------|-------|--------------------|
| Fields | string, text, richtext, code, date, select, link, file, image, signature, barcode, geo, rating, json, lookup-modal, etc. | 36 | ⚠️ Component compiler maps some; SSR uses Go templates |
| Layout | row, column, section, tabs, sheet, header, separator, button-box, html-block | 10 | ⚠️ Partial via component compiler |
| Views | list, form, kanban, calendar, gantt, map, tree, report, activity, view-editor | 10 | ⚠️ SSR renders HTML; components for SPA mode |
...
| Datatable | datatable, child-table, filter-bar, filter-panel, filter-builder, lookup-modal | 5 | ❌ Not wired |
| Dialogs | modal, confirm, alert, prompt, quick-entry, wizard, toast | 7 | ❌ Not wired |
| Search | search, favorites | 2 | ❌ Not wired |
| Widgets | badge, copy, phone, email, url, progress, statusbar, priority, handle, domain, placeholder, sync-status | 12 | ⚠️ Some via component compiler |
| Media | pdf, image, document, youtube, instagram, tiktok, video, audio | 8 | ✅ Standalone viewer/player components |
| Print | export, print, report-link | 3 | ❌ Not wired |
| Social | activity, chatter, timeline | 3 | ❌ Not wired |
| **Total** | | **122** | |

**Export architecture**: Post-build `generate-exports.mjs` creates group barrels in `dist/components/{group}/index.js`, `dist/full.js` (defineAll), per-group CSS, per-component CSS, theme CSS. 151 export paths in package.json. Component registry (`Registry.select()`) with dependency graph and include/exclude selector.

**Theme system**: `BcSetup.setTheme('light'|'dark'|'system')` / `onThemeChange()` / `setRtl()`. CSS custom properties (80+ tokens). `light.css` + `dark.css` theme files.
