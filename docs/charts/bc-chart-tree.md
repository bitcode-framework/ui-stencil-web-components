...
> Tree diagram (ECharts)

## Quick Start

```html
<bc-chart-tree data='{"name":"Root","children":[{"name":"Child A"},{"name":"Child B","children":[{"name":"Grandchild"}]}]}' />
```
...
|------|------|---------|-------------|
| data | string (JSON) | '[]' | Chart data — hierarchical `{name, children?}` |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| layout | string | 'orthogonal' | Layout type ('orthogonal' or 'radial') |
| orient | string | 'LR' | Orientation ('LR', 'RL', 'TB', 'BT') |
